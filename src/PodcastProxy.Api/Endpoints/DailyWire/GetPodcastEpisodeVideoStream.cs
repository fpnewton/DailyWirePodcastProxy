...
public class GetPodcastEpisodeVideoStreamRequest
{
    public required string Slug { get; set; }
}

public class GetPodcastEpisodeVideoStreamEndpoint(
    IDailyWireMiddlewareApi dwApiService,
    IDailyWireStreamApi dwStreamService
) : Endpoint<GetPodcastEpisodeVideoStreamRequest>
{
    public override void Configure()
    {
        Get("daily-wire/podcasts/episodes/{Slug}/streams/video");
    }

    public override async Task HandleAsync(GetPodcastEpisodeVideoStreamRequest req, CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(episode.Value.VideoUrl))
        {
        ...
        using var upstream = await dwStreamService.GetMediaStream(episode.Value.VideoUrl, headers =>
        {
